...
import { SavedLiftsDto, CompetitionCategoryDetailsDto } from "@/db/data-access/dto/lifts/types";

import { UserDto, UserInformationDto } from "@/db/data-access/dto/users/types";
import { getBenchmarkLiftsByUserId, getCompetitionCategoryDetails, getDefaultLiftId } from "@/db/data-access/lifts";
...
async function DashboardPage() {
  ...
  const competitionCategoryDetails: CompetitionCategoryDetailsDto[] = await getCompetitionCategoryDetails();

  const benchmarkLifts: SavedLiftsDto[] = await getBenchmarkLiftsByUserId(currentUser.id);
    ...
    <div className="container mx-auto pt-8 pb-24">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          <p className="text-base text-muted-foreground">Hello {currentUser.firstName} 👋</p>
          <p className="text-sm text-muted-foreground">Welcome to your dashboard. Here you can view your lifts progression, competition details and programming information.</p>
        </div>
        ...
        <LiftAnalyzer lifts={benchmarkLifts} userInformations={userInformations} defaultLiftId={defaultLiftId} />

        <CompetitionDetails currentTotal={currentTotal} competitionCategoryDetails={competitionCategoryDetails[0]} userInformations={userInformations} />
      </div>
    </div>
  );
}
