...
export const getEstimationLift = async (liftId: string): Promise<EstimationLiftDto | undefined> => {
    ...
    const liftEstimateLatestMax = liftsGroupedByLiftID[liftEstimate.liftForCalculationId].sort(
        (a, b) => new Date(b.oneRepMaxDate!).getTime() - new Date(a.oneRepMaxDate!).getTime()
    )[0].oneRepMax;
...
export const getCompetitionCategoryDetails = cache(async () => {
    ...
    const filteredCompetitionCategoryDetailsByAge: CompetitionCategoryDetails[] = competitionCategoryDetails.filter(
        (competitionCategoryDetail: CompetitionCategoryDetails) => {
            if (
                competitionCategoryDetail.maxBirthYear === null &&
                currentUserInformation.birthYear < competitionCategoryDetail.minBirthYear
            )
                return true;
    ...
    const filteredCompetitionCategoryDetailsByWeight: CompetitionCategoryDetails[] =
        filteredCompetitionCategoryDetailsByAge.filter((competitionCategoryDetail: CompetitionCategoryDetails) => {
            if (
    ...
    const filteredCompetitionCategoryDetailsByDivision: CompetitionCategoryDetails[] =
        filteredCompetitionCategoryDetailsByWeight.filter((competitionCategoryDetail: CompetitionCategoryDetails) => {
