...
import { User, UserInformation, UserLift, users, usersInformations, usersLifts } from '@/db/schema';
...
export const addUserInformation = async (userInformation: UserInformationDto) => {
    const userId = await getAuthenticatedUserId();
...
export const updateUserLiftUnitInformation = async ({ liftsUnit }: { liftsUnit: 'lbs' | 'kg' }) => {
    const userId = await getAuthenticatedUserId();
    await db.update(usersInformations).set({ liftsUnit: liftsUnit }).where(eq(usersInformations.userId, userId));
};

export const updateUserDivisionInformation = async ({ division }: { division: 'senior' | 'master' }) => {
    const userId = await getAuthenticatedUserId();
...
export const updateAccountSetupAt = async () => {
    ...
    await db.update(users).set({ accountIsSetup: new Date() }).where(eq(users.id, userId));
};
