...
import { MINIMUM_MASTER_BIRTH_YEAR, MINIMUM_SENIOR_BIRTH_YEAR } from '@/lib/constant';
...
export default function AccountSetupComponent() {
    const [isPending, setIsPending] = useState<boolean>(false);
    const [weightUnit, setWeightUnit] = useState<string>('kg');
    const [division, setDivision] = useState<'senior' | 'master' | 'junior'>('master');
    ...
    const handleAccountSetup = async (formData: FormData) => {
        ...
        const newUserAccountInformations: UserInformationDto = {
            ...
            weight: Number(convertWeightToKg(Number(formData.get('weight') as string), weightUnit)),
    ...
    const handleDivision = (value: string) => {
        ...
        if (birthYear <= MINIMUM_MASTER_BIRTH_YEAR) {
            setDivision('master');
        } else if (birthYear <= MINIMUM_SENIOR_BIRTH_YEAR && birthYear > MINIMUM_MASTER_BIRTH_YEAR) {
            setDivision('senior');
        } else {
            setDivision('junior');
        }
                ...
                <div className="grid gap-6">
                    ...
                    <div className="flex gap-4 items-center">
                        <p className="text-sm text-muted-foreground">
                            This is a one time process to setup your account. You will be asked to enter some details to
                            help us properly setup your account. These informations can be changed at any time by
                            visiting your account page and updating the informations under the &quot;Profile&quot;
                            section.
                        </p>
                    </div>
                </div>

                <form className="flex flex-col gap-4" action={handleAccountSetup}>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-col gap-6 bg-gradient-to-b from-neutral-800 p-4 rounded-xl w-full">
                            ...
                            <Select required name="birthYear" defaultValue={'1989'} onValueChange={handleDivision}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
                                        {Array.from({ length: 2009 - 1929 + 1 }, (_, i) => (
                                            <SelectItem value={(2009 - i).toString()} key={2009 - i}>
                                                {2009 - i}
                                            </SelectItem>
                                        ))}
                                    </SelectGroup>
                    ...
                    <Button type="submit" className="w-full rounded-xl mt-6" disabled={isPending}>
