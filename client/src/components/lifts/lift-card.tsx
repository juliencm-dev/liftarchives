    ...
    return (
        ...
        >
            <div className="flex flex-col gap-2 w-[55%] text-left">
                <div className="text-lg sm:text-2xl font-semibold text-foreground">{props.lift.lift.name}</div>
                <p className="text-xs text-muted-foreground">
                    {props.lift.date === null ? 'No data found' : new Date(props.lift.date).toDateString()}
                </p>
            </div>
            <div className="flex flex-col gap-2 text-right w-[calc(45%-1rem)]">
                <p className="text-sm sm:text-xl font-bold text-violet-200">
                    {props.lift.weight === null ? '--' : convertWeightToLbs(props.lift.weight, props.weightPreference)}{' '}
